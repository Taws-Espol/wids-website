...
export function Carrusel() {
	...
	return (
		<div className='m-0 h-[722px] w-full py-2 relative group'>
			...
			<div className='absolute bottom-[12%] left-1/2 transform -translate-x-1/2 translate-y-1/2 flex justify-around max-lg:bottom-[20%]'>
				<Timer Event_date={new Date("July 13, 2024 00:00:00")} />
			</div>
			<div className='absolute top-[10%] left-[5%] text-white'>
				<p className='text-4xl font-acumin font-bold'>
					{infoImages[currentIndex].titulo}
				</p>
				<p className='text-lg font-acumin font-normal'>
					{infoImages[currentIndex].fecha}
				</p>
				<p className='text-lg font-acumin font-normal'>
					{infoImages[currentIndex].lugar}
				</p>
			</div>
			<div className='absolute right-[5%] bottom-[5%] max-sm:left-[52%] max-md:left-[57%]  max-lg:left-[60%] max-lg:transform max-lg:-translate-x-1/2 max-lg:translate-y-1/2 '>
				<button className='text-white bg-blue-400 rounded-md px-10 py-3 text-center text-xl font-acumin font-medium'>
					Registro
				</button>
			</div>
		</div>
