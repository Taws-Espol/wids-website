...
export function ProfileCard({ conferencista, home, onClickFunction, active, year, animationClass = 'animate-fadeInFromBottom' }) {
    ...
    return (
        !boolean ?
            <div ref={cardRef} className={`transition-opacity duration-1000 ${isVisible ? `${animationClass}` : 'opacity-0'}`}>
                <Card className={`w-96 h-[600px] max-sm:w-80 ${home ? 'h-[600px] w-80' : ''} ${value ? 'opacity-40' : ''} hover:cursor-pointer transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300 hover:shadow-2xl`} onClick={() => onClickFunction(conferencista)} >
