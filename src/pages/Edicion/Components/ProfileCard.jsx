...
export function ProfileCard({
  ...
  return !boolean ? (
    ...
    >
      <Card
        className={`w-96 h-[600px] max-sm:w-80 ${home ? "h-[600px] w-80" : ""} ${value ? "opacity-40" : ""} hover:cursor-pointer transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-110 duration-300 hover:shadow-2xl`}
        onClick={() => onClickFunction(conferencista)}
      >
        <CardHeader
          floated={false}
          className={`min-h-96 w-full relative overflow-hidden ${home ? "h-60" : ""}`}
        >
          <img
            src={image}
            alt="Conferencista"
        ...
        <CardBody className="text-center h-32 text-primary-dark-green">
          <p className="text-primary-dark-green font-bold text-2xl">
            {conferencista.name}
          </p>
          <p>
    ...
    <Card
      className={`w-96 h-[580px] mt-5 max-sm:w-80 bg-neutral-grey bg-opacity-55 text-center place-content-center place-items-center`}
    >
